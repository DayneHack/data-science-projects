...
def main():
    ...
    X = PCA(heart.X, 2)             #dimension

    kmeans = KMeans(n_clusters, 'kmeans++', 300)
    ...
    #for i in range(10):
        #cluster = kmeans.fit(X)
        #avgSilhouette = avgSilhouette + kmeans.silhouette(cluster, X)
    #print(avgSilhouette / 10)
        
    #silhouette = kmeans.silhouette(cluster, X)
    #print(silhouette)
...
def visualize_cluster(x, y, clustering):
    ...
    plt.scatter(x, y, c=clustering, s=5)
    plt.show()
